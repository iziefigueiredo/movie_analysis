...
def set_theme():
    """Sets the visual theme for all plots."""
...
def add_value_labels_h(ax, values, fmt="{:.2f}", dx=0.02):
    """Adds value labels to a horizontal bar chart."""
...
def log1p_safe(s: pd.Series) -> pd.Series:
    """Applies a log1p transformation to a Series, handling non-numeric and negative values."""
    s = pd.to_numeric(s, errors="coerce").fillna(0)
    s = np.where(s < 0, 0, s)
    return pd.Series(np.log1p(s))

def simple_barh(index, values, xlabel, title, color=PALETTE["bar"]) -> plt.Axes:
...
def detect_genre_cols(df: pd.DataFrame) -> list[str]:
    """Detects dummy columns (0/1) as genres based on a subset of values."""
    candidates: list[str] = []
    for c in df.columns:
        lc = c.lower()
...
def melt_stars(df: pd.DataFrame) -> pd.DataFrame:
    """Transforms actor columns into a single 'actor' column."""
...
# ========================== VISUALIZATIONS =========================== #
def viz_hist_panel(df: pd.DataFrame, out: Path):
    """Generates a panel of histograms for key numeric variables."""
    fig, axes = plt.subplots(2, 2, figsize=(12, 8))
    if COL_RATING in df: axes[0,0].hist(pd.to_numeric(df[COL_RATING], errors="coerce").dropna(), bins=30, edgecolor="white"); axes[0,0].set_title("IMDB Rating")
    if COL_META   in df: axes[0,1].hist(pd.to_numeric(df[COL_META], errors="coerce").dropna(),   bins=30, edgecolor="white"); axes[0,1].set_title("Metascore")
    if COL_VOTES  in df: axes[1,0].hist(log1p_safe(df[COL_VOTES]).dropna(), bins=30, edgecolor="white"); axes[1,0].set_title("log1p(Number of Votes)")
    if COL_GROSS  in df: axes[1,1].hist(log1p_safe(df[COL_GROSS]).dropna(), bins=30, edgecolor="white"); axes[1,1].set_title("log1p(Gross Revenue)")
    for ax in axes.flat: ax.grid(alpha=0.3)
    fig.tight_layout(); fig.savefig(out, bbox_inches="tight"); plt.close(fig)

def viz_rating_by_genre(genre_summary: pd.DataFrame, out: Path, topn=10):
    """Creates a horizontal bar chart of average IMDB ratings by genre."""
    top = (genre_summary.sort_values("n_films", ascending=False)
           .head(topn).sort_values("mean_rating"))
    ax = simple_barh(top["genre"], top["mean_rating"], "Average IMDB Rating",
                     "Average Rating by Genre (Top by Count)")
    savefig(ax, out)

def viz_win_rate_by_genre(genre_summary: pd.DataFrame, out: Path, topn=10):
    """Creates a horizontal bar chart of Oscar win rates by genre."""
    top = (genre_summary.sort_values("oscar_win_rate", ascending=False)
           .head(topn).sort_values("oscar_win_rate"))
    ax = simple_barh(top["genre"], top["oscar_win_rate"],
                     "Win Rate (Wins / Films)",
                     "Top Genres by Oscar Win Rate",
                     color=PALETTE["bar2"])
    savefig(ax, out)

def viz_gross_by_oscar_nomination(df: pd.DataFrame, out: Path):
    """Compares average gross revenue for Oscar competitors vs. non-competitors."""
    if "oscar_nominations" not in df.columns:
        print("Warning: 'oscar_nominations' column not found. Skipping plot.")
        return

    df["Gross"] = pd.to_numeric(df["Gross"], errors="coerce").fillna(0)
    df["oscar_competitor"] = np.where(df["oscar_nominations"] > 0, "Competed", "Did Not Compete")
    gross_by_category = df.groupby("oscar_competitor")["Gross"].mean().sort_values()
    ...
    ax.set_title("Average Gross Revenue of Oscar Competitors vs. Non-Competitors")
    add_value_labels_h(ax, gross_by_category.values, fmt="${:,.0f}")
    
    savefig(ax, out)
    print(f"[ok] Generated gross revenue by Oscar competition chart: {out}")


def viz_box_rating_winners(df: pd.DataFrame, out: Path):
    """Creates a boxplot comparing IMDB ratings of Oscar winners vs. non-winners."""
    try:
        import seaborn as sns
    except Exception:
        print("[info] seaborn not available; skipping winners vs others boxplot.")
        return
    if COL_RATING not in df or COL_WINS not in df: return

    tmp = df.copy()
    tmp["winner_bin"] = (pd.to_numeric(tmp[COL_WINS], errors="coerce").fillna(0) > 0)\
                        .map({False:"Non-winner",True:"Winner"})
    ...
    sns.boxplot(
        ...
        legend=False,
        ax=ax,
        palette={"Non-winner":PALETTE["bar"], "Winner":PALETTE["bar2"]},
        showcaps=True,
        fliersize=0
    )
    ax.set_xlabel(""); ax.set_ylabel("IMDB Rating")
    savefig(ax, out, "IMDB Rating Distribution: Winners vs. Non-winners")
...
def viz_corr_heatmap(df: pd.DataFrame, out: Path, cols: list[str]):
    ...
    try:
        ...
        print("[info] seaborn/matplotlib extras not available; skipping heatmap.")
        return
    ...
    cmap = LinearSegmentedColormap.from_list(
        "pink_blue_div", ["#f472b6", "#0f172a", "#60a5fa"]
    )
    ...
    ax.set_title("Numeric Variable Correlation", pad=14, weight="semibold")
    ax.tick_params(axis="x", rotation=35); ax.tick_params(axis="y", rotation=0)
    savefig(ax, out)

def viz_actor_presence(df: pd.DataFrame, out: Path, topn=20):
    """Creates a horizontal bar chart of the most present actors."""
    long = melt_stars(df)
    if long.empty: return
    cnt = (long.groupby("actor")["film"].nunique()
                 .sort_values().tail(topn))
    ax = simple_barh(cnt.index, cnt.values, "Number of Films",
                     f"Top {topn} Actors by Film Count")
    savefig(ax, out)

def viz_actor_wins(df: pd.DataFrame, out: Path, topn=15):
    """Creates a horizontal bar chart of actors with the most Oscar wins."""
    long = melt_stars(df)
    if long.empty: return
    long["_won"] = (pd.to_numeric(long[COL_WINS], errors="coerce").fillna(0) > 0)
    wins = (long[long["_won"]].groupby("actor")["film"].nunique()
            .sort_values().tail(topn))
    ax = simple_barh(wins.index, wins.values, "Winning Films",
                     f"Top {topn} Actors by Oscar Wins",
                     color=PALETTE["bar2"])
    savefig(ax, out)

def viz_actor_mean_rating(df: pd.DataFrame, out: Path, min_films=3, topn=15):
    """Creates a horizontal bar chart of actors with the highest average IMDB rating."""
    long = melt_stars(df)
    if long.empty: return
    ...
    agg = (long.groupby("actor")
                ...
                .head(topn).sort_values("mean_rating"))
    if agg.empty: return
    ax = simple_barh(agg.index, agg["mean_rating"].values,
                     "Average IMDB Rating",
                     f"Actors with Highest Average Rating (Min. {min_films} Films)")
    savefig(ax, out)

def viz_actor_genre_heatmap(df: pd.DataFrame, out: Path,
                            min_films=5, topk_actors=20, topk_genres=10):
    """Creates a heatmap showing actor genre participation."""
    try:
        import seaborn as sns
        from matplotlib.colors import LinearSegmentedColormap
    except Exception:
        print("[info] seaborn not available; skipping actor-genre heatmap.")
        return
    ...
    sns.heatmap(pct, cmap=cmap, vmin=0, vmax=1, annot=False,
                cbar_kws={"shrink":0.85,"label":"participation (%)"})
    ax.set_xlabel("Genre"); ax.set_ylabel("Actor")
    ax.set_title("Actor-Genre Mix (Film Proportion)")
    savefig(ax, out)
...
def run(
    ...
    """Runs the full EDA pipeline to generate visualizations and summary statistics."""
    set_theme()
    ...
    df = pd.read_csv(inp)
    print(f"[ok] Base: {df.shape[0]} rows, {df.shape[1]} columns")

    # 1) Distributions
    viz_hist_panel(df, out_viz / "distributions.png")
    ...
    genre_summary = pd.DataFrame(genre_data).sort_values("n_films", ascending=False)
    ...
    # Adding the new visualization here
    ...
    if COL_VOTES in df.columns and COL_GROSS in df.columns:
        viz_scatter_reg(df, COL_VOTES, COL_GROSS, out_viz/"scatter_log_votes_log_gross.png",
                        logx=True, logy=True, title="log(Gross) x log(Votes)")
    ...
    # 5) Actors
    viz_actor_presence(df, out_viz/"actors_top_presence.png", topn=20)
    viz_actor_wins(df, out_viz/"actors_top_wins.png", topn=15)
    ...
    html = [
        ...
        "<p>Automatically generated visual summary. If a figure is missing, the corresponding column was likely not in the dataset.</p>",
    ]
    figs = [
        ("Distributions", "distributions.png"),
        ("Average Rating by Genre", "rating_by_genre.png"),
        ("Oscar Win Rate by Genre", "oscar_win_rate_by_genre.png"),
        ("Gross Revenue vs. Oscar Competition", "gross_by_oscar_nomination.png"),
        ("IMDB Rating: Winners vs. Non-winners", "box_imdb_winner_vs_other.png"),
        ("log(Gross) x log(Votes)", "scatter_log_votes_log_gross.png"),
        ("Numeric Variable Correlation", "corr_numeric.png"),
        ("Actors – Most Present", "actors_top_presence.png"),
        ("Actors – Most Wins", "actors_top_wins.png"),
        ("Actors – Highest Average Rating", "actors_best_mean_rating.png"),
        ("Actors x Genre – Participation", "actors_genre_heatmap.png"),
    ]
    ...
    html += [
        "<hr/><p style='opacity:.7'>Generated by <code>src/eda_imdb_oscar.py</code></p>",
        "</body></html>"
    ]
...
def parse_args():
    ...
    ap = argparse.ArgumentParser(description="EDA of imdb_oscar: genres (dummies) and actors.")
