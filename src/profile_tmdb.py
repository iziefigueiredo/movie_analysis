...
INP_DEFAULT = "data/processed/tmdb_clean.csv"
OUT_DIR_DEFAULT = "reports"

def run_profile(
    inp: str = INP_DEFAULT,
    out_dir: str = OUT_DIR_DEFAULT,
    html_name: str = "eda_tmdb_profile.html",
    ...
    kwargs = {
        "title": "TMDB – EDA com ydata-profiling",
        "minimal": minimal,
...
def parse_args():
    p = argparse.ArgumentParser(description="Gera EDA do TMDB com ydata-profiling")
    p.add_argument("--inp", default=INP_DEFAULT)
    p.add_argument("--out_dir", default=OUT_DIR_DEFAULT)
    p.add_argument("--html_name", default="eda_tmdb_profile.html")
